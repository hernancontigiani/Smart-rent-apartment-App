...
Reporte Nº
0: Get all reports
1: Numbers of rents available per rooms
2: Rent price per rooms
3: Numbers of rents available per size(m2)
4: Rent price per size (m2)
5: Prediction about the relation between price and size
6: Numbers of rents and rent price per rooms
7: Numbers of rents and rent price per size (m2)
...
def generate(reporte, silent_mode=False):
...
if __name__ == '__main__':
    ...
    if(reporte < 0 or reporte > 7):
        print("Error: Repors available between 0 y 7")

    generate(reporte=reporte)